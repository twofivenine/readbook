"use client";
/** 책 후보 추천 · 수기 입력 (F-9) + 후보 10권 초과 시 삭제 선택 (F-2.3) */
import Link from "next/link";
import { useRouter } from "next/navigation";
import { useRef, useState } from "react";
import { api } from "@/lib/api";
import { usePoll, useWrite } from "@/lib/hooks";
import { Button, Card, Divider, ErrorText, Input, Label, Title } from "@/components/ui";
  ...
  return (
    <div className="flex flex-col gap-4 max-w-[520px] mx-auto">
      <div className="flex items-center gap-3">
        <Link href="/polls/book" className="text-[17px] text-muted hover:text-ink" aria-label="투표로">←</Link>
        <Title>책 후보 추천</Title>
      </div>
      {full ? (
        <Card>
          <Title size="sm">기존 후보 1권을 삭제해야 합니다</Title>
          <div className="text-[13.5px] text-muted">후보가 {limit}권으로 찼어요. 삭제할 후보를 고르세요. 그 후보의 체크 수도 함께 사라집니다.</div>
          <ul className="flex flex-col gap-2">
            {poll.data?.candidates.map((c) => (
              <li key={c.id}>
                <button type="button" disabled={del.isPending || closed} onClick={() => window.confirm(`"${c.book?.title}" 후보를 삭제할까요? 체크 ${c.checkCount}개가 함께 사라져요.`) && del.mutate(c.id)}
                  className="w-full flex justify-between gap-3 rounded-[10px] border border-line px-3.5 py-3 text-left hover:border-danger transition-colors">
                  <span className="truncate text-[14px]">{c.book?.title}</span>
                  <span className="text-[12px] text-mono shrink-0">체크 {c.checkCount} · 삭제</span>
                </button>
              </li>
            ))}
          </ul>
          <ErrorText>{del.error?.message}</ErrorText>
          <Label>삭제하면 아래 추천 폼이 열려요</Label>
        </Card>
      ) : (
        <Card className="gap-4">
          {closed && <div className="text-[13.5px] text-danger">투표가 마감되어 있어요. 재투표를 열면 추천할 수 있어요.</div>}
          <form className="flex flex-col gap-4" onSubmit={(e) => { e.preventDefault(); submit.mutate(undefined, { onSuccess: () => router.push("/polls/book") }); }}>
            <div className="flex gap-4">
              <button type="button" onClick={() => fileRef.current?.click()} className="w-[104px] shrink-0 rounded-md border border-dashed border-[#d3cec3] cover-placeholder flex flex-col items-center justify-center gap-1 overflow-hidden hover:border-accent" style={{ aspectRatio: "3 / 4" }} aria-label="표지 업로드">
                {preview ? (
            ...
            <Divider />
            <div className="text-[12px] text-mono leading-relaxed">
              업로드한 표지의 저작권 책임은 업로더에게 있습니다.<br />
              외부 도서 API 연동 없음 — 직접 입력합니다. JPEG/PNG/WebP · 5MB 이하 · 자동 리사이즈
            </div>
            <ErrorText>{submit.error?.message}</ErrorText>
            <Button type="submit" variant="primary" full disabled={submit.isPending || closed || !title.trim() || !author.trim() || !pages}>{submit.isPending ? "등록 중…" : "후보로 추천"}</Button>
          </form>
        </Card>
      )}
    </div>
