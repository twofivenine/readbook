"use client";
/** F-6.3 스포일러: 가림 + 탭으로 펼침 (펼침 상태는 컴포넌트 로컬) */
...
export function Spoiler({ hidden, children, className }: { hidden: boolean; children: React.ReactNode; className?: string }) {
  ...
  return (
    <button type="button" onClick={() => setOpen(true)} className={clsx("text-left inline-flex items-center gap-2 rounded-md bg-bg px-2 py-1 border-[1.5px] border-dashed border-line", className)} aria-label="스포일러 펼치기">
